...
* с этим. Нужно либо сделать регулярные выражения заранее скомпилированными,
* либо заменить регулярные выражения на обычные функции. Если оставить
* регулярные выражения, то нужно сделать шаблоны с ними, чтобы они были уже
* скомпилированными для разных используемых типов переменных.
...
namespace XMLB
{
	...
	const std::regex kTag_Element_template{ R"(<([^<>]+)>)"};
	...
	const std::regex kText_sign_template{ R"(>(.+)<)" };

	/// Шаблон регулярного выражения для нахождения значения, текста тега
	const std::regex kText_template{ R"(\S+)" };
